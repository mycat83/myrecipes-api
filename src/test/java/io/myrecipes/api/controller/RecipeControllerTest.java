...
public class RecipeControllerTest {
    ...
    public void Should_첫페이지_조회_When_쿼리스트링_없이_레시피_페이지_호출() throws Exception {
        RecipeDTO recipeDTO = RecipeDTO.builder().title("test1").image("image1.jpg").estimatedTime(30).difficulty(1).build();
        given(this.recipeService.readRecipePageSortedByParam(eq(0), eq(10), eq("registerDate"), eq(false))).willReturn(Collections.singletonList(recipeDTO));
